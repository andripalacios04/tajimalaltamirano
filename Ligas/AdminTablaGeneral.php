...
$orden = isset($_GET['orden']) ? trim($_GET['orden']) : 'puntos_desc';

$ordenSql = "cg.puntos DESC, cg.diferencia_goles DESC, cg.goles_favor DESC, e.Nombre ASC";
$ordenLabel = "Puntos (Mayor a menor)";

if ($orden === 'equipo_az') {
    $ordenSql = "e.Nombre ASC";
    $ordenLabel = "Equipo (A - Z)";
} elseif ($orden === 'equipo_za') {
    $ordenSql = "e.Nombre DESC";
    $ordenLabel = "Equipo (Z - A)";
} elseif ($orden === 'puntos_asc') {
    $ordenSql = "cg.puntos ASC, cg.diferencia_goles ASC, cg.goles_favor ASC, e.Nombre ASC";
    $ordenLabel = "Puntos (Menor a mayor)";
}
...
$sql = "SELECT 
            ...
            cg.partidos_jugados,
            cg.partidos_ganados,
            cg.partidos_empatados,
            cg.partidos_perdidos,
            cg.goles_favor,
            cg.goles_contra,
            cg.diferencia_goles,
            cg.sets_ganados,
            cg.sets_perdidos,
            cg.tarjetas_amarillas,
            cg.tarjetas_rojas,
            cg.faltas_cometidas,
            cg.Jornada
        FROM clasificacion_general cg
        INNER JOIN equipo e ON e.Id_Equipo = cg.id_equipo
        WHERE cg.id_liga = ?
        ORDER BY $ordenSql";
...
<head>
<meta charset="UTF-8">
<title>Tabla General - RETAME</title>
...
table{width:100%;border-collapse:collapse;min-width:980px;background:rgba(0,0,0,0.22);}
...
.team{text-align:left;font-weight:900;}
.pos{font-weight:900;}
...
<?php if (function_exists('retame_render_shell')) { retame_render_shell('Tabla General - RETAME'); } ?>
...
<div class="main-content">
    ...
    <div class="card">
        ...
        <form class="controls" method="GET" action="">
            ...
            <select name="orden">
                <option value="puntos_desc" <?php echo ($orden==='puntos_desc')?'selected':''; ?>>Puntos (Mayor a menor)</option>
                <option value="puntos_asc" <?php echo ($orden==='puntos_asc')?'selected':''; ?>>Puntos (Menor a mayor)</option>
            ...
            <a class="btn btn-secondary" href="AdminTablaGeneral.php?id_liga=<?php echo h($id_liga); ?>&id_adminsolicitud=<?php echo h($id_adminsolicitud); ?>">Reset</a>
        </form>

        <?php if (empty($tabla)): ?>
            <div class="empty">No hay registros de clasificación general para esta liga.</div>
        <?php else: ?>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="team"><?php echo h($r['equipo_nombre']); ?></td>
                            <td><b><?php echo (int)$r['puntos']; ?></b></td>
                            <td><?php echo (int)$r['partidos_jugados']; ?></td>
                            <td><?php echo (int)$r['partidos_ganados']; ?></td>
                            <td><?php echo (int)$r['partidos_empatados']; ?></td>
                            <td><?php echo (int)$r['partidos_perdidos']; ?></td>
                            <td><?php echo (int)$r['goles_favor']; ?></td>
                            <td><?php echo (int)$r['goles_contra']; ?></td>
                            <td><?php echo (int)$r['diferencia_goles']; ?></td>
                            <td><?php echo h($r['Jornada']); ?></td>
                        </tr>
                        <?php
                            $pos++;
                        endforeach;
            ...
            <div class="subtext" style="margin-top:14px;">
                Mostrando: <b><?php echo count($tabla); ?></b> equipos
            </div>
        <?php endif; ?>
