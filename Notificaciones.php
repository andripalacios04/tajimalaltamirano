...
function esMiembroEquipo($conn, $idEquipo, $idUsuario) {
    $sql = "SELECT 1 FROM equipo_jugador WHERE Id_Equipo = ? AND Id_Jugador = ? LIMIT 1";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ss", $idEquipo, $idUsuario);
    $stmt->execute();
    $stmt->store_result();
    $ok = ($stmt->num_rows > 0);
    $stmt->close();
...
function esCapitanOEntrenador($equipoRow, $idUsuario) {
    if (!$equipoRow) return false;
    return (($equipoRow['Capitan'] ?? '') === $idUsuario) || (($equipoRow['Entrenador'] ?? '') === $idUsuario);
}

function extraerIdDesdeDescripcion($descripcion) {
...
if (!empty($Id_Retador)) {
    $sqlPerfilVisual = "SELECT ModoPerfil, FotoPerfil FROM retador WHERE Id_Retador = ? LIMIT 1";
    ...
    if ($stmtPerfilVisual) {
        ...
        if ($filaPerfilVisual = $resPerfilVisual->fetch_assoc()) {
            ...
            $fotoPerfilBD = isset($filaPerfilVisual['FotoPerfil']) ? trim((string)$filaPerfilVisual['FotoPerfil']) : '';
            $FotoPerfilUsuario = resolverFotoPerfilRetame($fotoPerfilBD);
        }
...
if ($Id_Retador === '') {
    ...
    $sqlNoti = "SELECT 
                ...
                WHERE n.id_retador = ?
                ORDER BY 
                    CASE WHEN UPPER(COALESCE(n.Estado,''))='VER' THEN 0 ELSE 1 END,
                    n.fecha DESC";
    $stmtNoti = $conn->prepare($sqlNoti);
    $stmtNoti->bind_param("s", $Id_Retador);
    $stmtNoti->execute();
    $resNoti = $stmtNoti->get_result();

    while ($row = $resNoti->fetch_assoc()) {

        $idEquipo = $row['id_area'];
        $tipo = isset($row['tipo']) ? trim($row['tipo']) : '';
        $estadoActual = isset($row['Estado']) ? trim($row['Estado']) : '';
        $estadoNorm = strtoupper($estadoActual === '' ? 'VER' : $estadoActual);
        ...
        $esMiembro = esMiembroEquipo($conn, $idEquipo, $Id_Retador);
        ...
        $descripcionMostrar = (string)($row['descripcion'] ?? '');
        if ($tipo === 'Aceptacion Equipo') {
            $idQuienSeUnio = extraerIdDesdeDescripcion($descripcionMostrar);
            if ($idQuienSeUnio !== '' && $idQuienSeUnio === $Id_Retador) {
                $descripcionMostrar = ($row['nombre_equipo'] ?? 'El equipo') . " acepto tu solicitud.";
            }
        }

        $row['descripcion_mostrar'] = $descripcionMostrar;
        $row['Estado_original'] = ($estadoNorm === 'VER') ? 'Ver' : 'Visto';
        $row['puede_ver_solicitudes'] = ($tipo === 'Solicitud Equipo' && esCapitanOEntrenador($equipoRow, $Id_Retador));

        if ($estadoNorm === 'VER') {
            $sqlUpd = "UPDATE notificaciones 
                       SET Estado = 'Visto' 
                       WHERE Id_notificacion = ? AND id_retador = ? AND (Estado IS NULL OR UPPER(Estado)='VER')";
            $stU = $conn->prepare($sqlUpd);
            $stU->bind_param("ss", $row['Id_notificacion'], $Id_Retador);
            $stU->execute();
            $stU->close();
            $row['Estado'] = 'Visto';
        } else {
            $row['Estado'] = $estadoActual;
        }
...
<head>
<meta charset="UTF-8">
<title>Notificaciones - RETAME</title>
...
<header class="topbar">
    ...
    <div class="topbar-title">
        <h1>Notificaciones de <?php echo esc($Nombre); ?></h1>
    </div>
...
<main class="main-content">
    <div class="notificaciones-page">
        <section class="perfil-card noti-hero">
            <span class="noti-kicker">🔔 Centro de avisos</span>
            <h3>NOTIFICACIONES</h3>
            <p>Consulta tus avisos de equipos, solicitudes y movimientos importantes dentro de RETAME.</p>
            ...
            <div class="noti-resumen">
                ...
                <div class="noti-stat">
                    <span>Nuevas</span>
                    <strong><?php echo (int)$nuevas_count; ?></strong>
                </div>
        ...
        <?php if ($error !== ''): ?>
            <div class="alert"><strong>⚠️ Error:</strong> <?php echo esc($error); ?></div>
        <?php endif; ?>

        <section class="noti-panel">
            <div class="noti-panel-header">
                <div class="noti-panel-title">📩 Mis notificaciones</div>
                <div class="noti-panel-count"><?php echo count($notificaciones); ?> avisos</div>
            </div>

            <?php if (empty($notificaciones)): ?>
                <div class="noti-empty">No tienes notificaciones por ahora.</div>
            <?php else: ?>
                <div class="noti-list">
                    <?php foreach ($notificaciones as $n): ?>
                        <?php
                            $tipoTxt = esc($n['tipo']);
                            $desc = esc($n['descripcion_mostrar']);
                            $fechaTxt = esc($n['fecha']);
                            $estadoOriginal = isset($n['Estado_original']) ? $n['Estado_original'] : 'Visto';
                            $cls = (strtoupper($estadoOriginal) === 'VER') ? 'noti-ver' : 'noti-visto';
                            $puedeVer = !empty($n['puede_ver_solicitudes']);
                        ?>
                        <div class="noti-item <?php echo $cls; ?>">
                            <div class="noti-top">
                                <span class="noti-badge"><?php echo $tipoTxt; ?></span>
                                <span class="noti-date"><?php echo $fechaTxt; ?></span>
                            </div>

                            <div class="noti-desc"><?php echo $desc; ?></div>

                            <?php if (trim($n['tipo']) === 'Solicitud Equipo' && $puedeVer): ?>
                                <div class="noti-actions">
                                    <a class="btn btn-primary" href="Solicitudes.php">Ver solicitud</a>
                                </div>
                            <?php endif; ?>
                        </div>
                    <?php endforeach; ?>
...
<nav class="bottom-nav">
    ...
    <a href="Notificaciones.php" class="active" aria-label="Notificaciones">
